  ...
  angular.module('dpd', []).value('dpdConfig', [])
    ...
    .factory('dpd', ['$http', '$rootScope', 'dpdConfig', 'dpdSocket', function ($http, $rootScope, dpdConfig, dpdSocket) {
      ...
      dpdConfig.collections.forEach(function (d) {
        ...
        dpd[d].exec = function (method, funcName, data, successCallback, errorCallback) {
          // method parameter is optional
          if (typeof method === "string" && typeof funcName === "object") {
          ...
          return $http(options).success(checkUndefinedFunc(successCallback)).error(ef).error(checkUndefinedFunc(errorCallback));
        };
